...
class find_shrimp_features:
    ...
    # Function to detect the top left corner from a chessboard
    def find_chessboard_tl_corner(self, frame, col, row):
        # Convert the frame to grayscale
        #gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        chessboard_size = (col, row)
        ret, corners = cv2.findChessboardCorners(frame, chessboard_size, None)
        if ret:
            p0 = corners[0]
            cv2.circle(frame, (int(p0[0,0]), int(p0[0,1])), 5, (0,0,255), 3)
            return True, p0[0,0], p0[0,1]
        else:
            return False, None, None
